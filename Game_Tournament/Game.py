...
def load_game_from_json(filename):
    """ Load a game from a JSON file and return it as a Game object. """
...
def a_tournament():
    ...
    players_mex = ['Chicharito','Piojo','Guardado','Hector Moreno','Rafa Marquez','Salcido','Vela','Dos Santos','Herrera','Layun','Corona']
    players_arg = ['Messi','Di Maria','Aguero','Higuain','Mascherano','Biglia','Dybala','Paredes','Tagliafico','Otamendi','Zabaleta']
    players_peru = ['Guerrero','Carrillo','Flores','Yotun','Tapia','Cueva','Advincula','Trauco','Araujo','Gallese','Santamaria']
    players_brazil = ['Neymar','Firmino','Coutinho','Casemiro','Paulinho','Willian','Alisson','Thiago Silva','Marquinhos','Danilo','Alex Sandro']
    players_france = ['Mbappe','Griezmann','Pogba','Kante','Varane','Lloris','Matuidi','Giroud','Mendy','Fekir','Pavard']  
    players_germany = ['Muller','Kroos','Reus','Hummels','Neuer','Draxler','Gomez','Khedira','Boateng','Gotze','Werner']
    players_spain = ['Ramos','Iniesta','Pique','Busquets','Isco','Silva','Alba','Carvajal','Asensio','Koke','De Gea']
    player_italy = ['Buffon','Chiellini','Bonucci','Donnarumma','Insigne','Belotti','Jorginho','Verratti','Bernardeschi','Chiesa','Immobile']
    sport = Sport("Futbol", 11, "FIFA")
    team_mex = Team("Mexico", sport)
    team_arg = Team("Argentina", sport)
    team_pero = Team("Peru", sport)
    ...
    for player in players_peru:
        team_pero.add_athlete(Athlete(player))
